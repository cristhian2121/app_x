...
import { getStudentMessages, getDressMakerInfo } from '../uniformsApi/uniformsAPI.js';
...
const info = {"_id":"63b8ed901b7fc428d0dc0f63","firstName":"Steffen","lastName":null,"nickName":"siorizzi0@adobe.com","gender":null,"shirtSize":"XS","password":"QyscVvDYE78I","deliveryDate":null,"__v":0};

const Prueba = () => {

  const userID = "63b8ed901b7fc428d0dc0f64";

  const {isLoading, data: studentMessages, isError, error} = useQuery({
    queryKey: ['StudentMessages', userID],
    queryFn: () => getStudentMessages(userID),
    ...
    const queries = unique?.map( id => ({
      ...
      const users = modistasInfo.map(query => query.data);

      console.log(modistasInfo)

  // const queries = studentMessages?.map(userId => ({
  ...
  const enviarMensajes = (message) => {
    console.log(message)
     ...
     if (isLoading)
       return (
         <div>
           <h1>Estan cargando los datos...</h1>
         </div>
       );
     else if (isError)
       return (
         <div>
           <h1>Error papa {error.message}</h1>
         </div>
       );

  // if (modistasInfo.some(query => query.isLoading)) {
  //   return <div>Loading...</div>;
  // }

  const paraprobar = modistasInfo.some(query => query.isLoading);
  console.log(paraprobar)
  return (
    <Container
      sx={{
        width: "80%",
